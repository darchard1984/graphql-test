    ...
    <nav class="uk-navbar-container" uk-navbar>
      <div class="uk-navbar-left">
        <ul class="uk-navbar-nav">
          ...
          <li>
            <a href="/">Motorcycle Lifestyle Blog </a>
          </li>
        </ul>
      </div>
...
export default defineComponent({
  setup() {
    const { $apollo } = useContext()
    const categories: Ref<Array<{ name: string; id: number }>> = ref([])
    const { fetch } = useFetch(async () => {
      const r = await $apollo.query({ query: CATEGORIES_QUERY })
